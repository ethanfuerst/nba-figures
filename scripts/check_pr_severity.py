...
- major: commits with `!` suffix or BREAKING CHANGE footer
- minor: `feat:` commits
- patch: `fix:`, `chore:`, `refactor:`, etc.
...
def check_for_breaking_change_footer(commit_hash: str) -> bool:
    ...
    return bool(re.search(r'^BREAKING[ -]CHANGE\s*:', result.stdout, re.MULTILINE))
...
def main() -> int:
    ...
    parser.add_argument('--pr-title', required=True, help='The PR title to check')
    parser.add_argument(
        '--base-ref',
        required=True,
    ...
    pr_severity = get_commit_severity(args.pr_title)
