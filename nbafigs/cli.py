...
@click.option('--seasons', '-s', type=int, multiple=True, help='Season year(s), e.g. 2023 or 2020 2023')
@click.option('--kind', type=click.Choice(['normal', 'hex']), default='normal')
@click.option('--scale', type=click.Choice(['P_PPS', 'PCT_DIFF', 'D_PPS']), default='P_PPS')
...
@click.option('--output', '-o', type=click.Path(), default=None, help='Save to file instead of displaying')
@click.option('--opponent', type=str, default=None, help='Opponent team abbreviation (e.g. DAL)')
...
@click.option('--game-segment', type=click.Choice(['First Half', 'Overtime', 'Second Half']), default=None)
@click.option('--season-segment', type=click.Choice(['Post All-Star', 'Pre All-Star']), default=None)
...
def player(player_name, seasons, kind, scale, no_misses, no_pct, title, context, output, **filters):
    ...
    df, fig = player_shot_chart(player_name, seasons=seasons_list, chart_params=chart_params, **limiters)
...
def team(team_abbrev, game_id, playoffs, kind, scale, no_misses, no_pct, output):
    ...
    df, fig = team_shot_chart(team_abbrev, game_id, playoffs=playoffs, chart_params=chart_params)
